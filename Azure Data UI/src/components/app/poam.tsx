...
export function PoamSection({
  ...
  const counts = {
    total: items.length,
    open: items.filter((i) => !closedStatuses.includes(i.status)).length,
    overdue: items.filter((i) => !closedStatuses.includes(i.status) && daysOut(i.scheduledCompletion) < 0)
      .length,
    milestones: items.reduce(
      (n, i) => n + i.milestones.filter((m) => m.status !== "Completed").length,
      0,
    ),
  };
  ...
  return (
    <>
      <div className="grid grid-cols-2 border-y border-border sm:grid-cols-4">
        {[
          { label: "POA&M items", value: String(counts.total), sub: "OSCAL poam-item entries" },
          { label: "Open", value: String(counts.open), sub: "Not completed or accepted" },
          {
            label: "Overdue",
            value: String(counts.overdue),
            sub: counts.overdue > 0 ? "Past scheduled completion" : "None past scheduled date",
          },
          {
            label: "Outstanding milestones",
            value: String(counts.milestones),
            sub: "Across all open items",
          },
        ].map((s) => (
          <div
            key={s.label}
          ...
          >
            ...
            <div className="tnum mt-0.5 text-[17px] font-semibold tracking-[-0.01em]">{s.value}</div>
